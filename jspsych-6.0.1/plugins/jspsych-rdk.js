...
jsPsych.plugins["RDK"] = (function() {
	...
	plugin.trial = function(display_element, trial) {
		...
		*/
		...
		var display_element_position = display_element.style.position
		...
		//this was added by me to make the stimuli flush with the screen	
		...
		function end_trial() {
			...
			display_element.style.position = display_element_position
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
		    ...
		    if(fixationCross === true){
		      ...
		      ctx.moveTo(trial.aperture_center_x - fixationCrossWidth, trial.aperture_center_y);
		      ctx.lineTo(trial.aperture_center_x + fixationCrossWidth, trial.aperture_center_y);
		      ...
		      ctx.beginPath();
		      ctx.lineWidth = fixationCrossThickness;
		      ctx.moveTo(trial.aperture_center_x, trial.aperture_center_y - fixationCrossHeight);
		      ctx.lineTo(trial.aperture_center_x, trial.aperture_center_y + fixationCrossHeight);
		      ctx.fillStyle = fixationCrossColor;
		      ctx.stroke();
		    }
	      	...
	      	if(border === true){
	        	...
	        	if(apertureType === 1 || apertureType === 2){
	          		...
	          		ctx.beginPath();
	          		ctx.ellipse(window.innerWidth*(1/4), apertureCenterY, horizontalAxis+(borderThickness/2), verticalAxis+(borderThickness/2), 0, 0, Math.PI*2);
	          		ctx.stroke();
	          		
					ctx.lineWidth = borderThickness;
	          		ctx.strokeStyle = borderColor;
	          		ctx.beginPath();
	          		ctx.ellipse(window.innerWidth*(3/4), apertureCenterY, horizontalAxis+(borderThickness/2), verticalAxis+(borderThickness/2), 0, 0, Math.PI*2);
	          		ctx.stroke();
