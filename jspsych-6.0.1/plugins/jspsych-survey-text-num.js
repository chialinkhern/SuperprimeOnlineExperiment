...
jsPsych.plugins['survey-text-num'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-text-num',
    description: '',
    parameters: {
	  trial_duration: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input type="text" style="font-size:22pt" name="#jspsych-survey-text-response-' + i + '"autocomplete = "off" onkeypress="return noenter()" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'" '+autofocus+' '+req+'></input>';
      } else {
    ...
    display_element.querySelector('#jspsych-survey-text-form').addEventListener('submit', function() {
      ...
      // next trial when time limit is done
